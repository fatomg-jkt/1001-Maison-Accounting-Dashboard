...
import {BUILD_SESSION_SECRET} from './generated-session-secret.js'
...
const sessionSecret=()=>process.env.SESSION_SECRET||BUILD_SESSION_SECRET
const encode=(value:string)=>Buffer.from(value).toString('base64url')
const signature=(value:string,secret=sessionSecret())=>crypto.createHmac('sha256',secret).update(value).digest('base64url')
export function createSession(user:SessionIdentity,now=Date.now(),secret=sessionSecret(),maxAge=SESSION_SECONDS){if(!secret)throw new Error('Session secret belum tersedia.');const payload=encode(JSON.stringify({...user,exp:Math.floor(now/1000)+maxAge}));return `${payload}.${signature(payload,secret)}`}
export function verifySession(token:string|undefined,now=Date.now(),secret=sessionSecret()):SessionPayload|null{if(!token||!secret)return null;const [payload,sig,...rest]=token.split('.');if(!payload||!sig||rest.length||signature(payload,secret)!==sig)return null;try{const parsed=JSON.parse(Buffer.from(payload,'base64url').toString()) as SessionPayload;return parsed.exp>Math.floor(now/1000)?parsed:null}catch{return null}}
