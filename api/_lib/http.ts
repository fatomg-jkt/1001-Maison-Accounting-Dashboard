...
export function failure(res:any,error:unknown){const detail=error instanceof Error?error.message:String(error??'');const configuration=detail.startsWith('Konfigurasi server belum lengkap:');const blobDataError=detail.startsWith('Data pengguna pada Vercel Blob')||detail.startsWith('Stream data pengguna pada Vercel Blob');const development=process.env.NODE_ENV!=='production';send(res,configuration?503:500,{message:configuration||blobDataError?detail:'Terjadi kesalahan server.',...(development&&detail?{detail}: {})})}
