...
export function createSession(user:SessionIdentity,now=Date.now(),secret=process.env.SESSION_SECRET!){const payload=encode(JSON.stringify({...user,exp:Math.floor(now/1000)+SESSION_SECONDS}));return `${payload}.${signature(payload,secret)}`}
...
export const cookie=(token:string,production=process.env.NODE_ENV==='production')=>`${COOKIE_NAME}=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_SECONDS}${production?'; Secure':''}`
